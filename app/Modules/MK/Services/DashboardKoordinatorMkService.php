...
/**
 * Data dashboard Koordinator Mata Kuliah: KPI serta capaian tertinggi
 * discope ke MK yang dikoordinatori user (bukan seluruh unit), meniru pola
 * App\Modules\Kurikulum\Services\DashboardTimKurikulumService.
 */
class DashboardKoordinatorMkService
{
    use HasKoordinatorMkScope;

    /** Jumlah MK yang dikoordinatori user (mk.koordinator_mk_id atau kelas_mk.koordinator_mk_id). */
    public function jumlahMk(User $user): int
    {
        return static::scopedKoordinatorMkIds($user)->count();
    }

    /**
     * CPL dengan rerata capaian tertinggi, dari MK yang dikoordinatori user.
     *
